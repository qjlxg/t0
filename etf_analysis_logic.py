...
def analyze_single_file(file_path, etf_names):
    try:
        ...
        if len(df) < 120: return None # 至少需要半年数据计算趋势
        ...
        if curr_down >= 2:
            if is_bull:
                # 顺势：只要连跌就给分，乖离大给高分
                score = curr_down + (2 if bias20 < -5 else 0)
                rating = f"🔴顺势捡钱 {'⭐'*score}"
                prio = 3
            else:
                # 逆势：必须连跌3天以上或乖离够大才给信号
                if curr_down >= 4 or bias20 < -7:
...
def main():
    ...
    res_df['prio'] = prios
    # 优先级排序：红色 > 蓝色 > 黄色 > 灰色
    res_df = res_df.sort_values(['prio', '3跌胜率%'], ascending=False).drop(columns=['prio'])

    out = f"etf_signal_{datetime.now().strftime('%Y%m%d')}.csv"
    res_df.to_csv(out, index=False, encoding='utf_8_sig')
    print(f"报告已生成：{out}。红色代表安全机会，蓝色代表高危反弹，黄色代表避坑。")
